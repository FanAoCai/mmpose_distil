...
from mmcv.runner import (DistSamplerSeedHook, EpochBasedRunner, OptimizerHook,
                         get_dist_info)
from ..core.runner import DistillationRunner, DistilRunner, TransformerRunner
...
def distil_train_model(model,
    ...
    # prepare data loaders
    dataset = dataset if isinstance(dataset, (list, tuple)) else [dataset]
    teacher_dataset = teacher_dataset if isinstance(teacher_dataset, (list, tuple)) else [teacher_dataset]
    ...
    data_loaders = [build_dataloader(ds, **train_loader_cfg) for ds in dataset]
    teacher_data_loaders = [build_dataloader(ds, **train_loader_cfg) for ds in teacher_dataset]
    ...
    if cfg.resume_from:
        ...
        runner.load_checkpoint(cfg.load_from)
    runner.run(data_loaders, teacher_data_loaders, cfg.workflow, cfg.total_epochs)

    

def transformer_train_model(model,
                dataset,
                cfg,
                distributed=False,
                validate=False,
