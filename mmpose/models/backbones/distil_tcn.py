...
class BasicTemporalBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if use_stride_conv:
            # print('use_stride_conv !')
            self.stride = kernel_size
            self.causal_shift = kernel_size // 2 if causal else 0
            self.dilation = 1
            # print('kernel_size :', kernel_size)
            # print('causal :', causal)
            # print('causal_shift :', self.causal_shift)
        else:
            # print('no_use_stride_conv !')
            self.stride = 1
            self.causal_shift = kernel_size // 2 * dilation if causal else 0
            # print('no_kernel_size :', kernel_size)
            # print('no_causal :', causal)
            # print('no_causal_shift :', self.causal_shift)
    ...
    def forward(self, x):
        ...
        if self.use_stride_conv:
            # print('use_stride_conv : ',self.use_stride_conv)
            # print('self.causal_shift : ', self.causal_shift)
            # print('self.kernel_size : ', self.kernel_size)
            # print('x.shape[2] : ', x.shape[2])
            assert self.causal_shift + self.kernel_size // 2 < x.shape[2]
        else:
            # print('else_self.pad : ',self.pad)
            # print('else_self.causal_shift : ', self.causal_shift)
            # print('else_self.kernel_size : ', self.kernel_size)
            # print('else_x.shape[2] : ', x.shape[2])
