...
import torch
from mmpose.models.detectors.pose_lifter import PoseLifter
from .distil_base import DistilBasePose



@POSENETS.register_module()
class DistilPoseLifter(DistilBasePose):
    ...
    def __init__(self,
        ...
        if self.semi:
            ...
            self.losds_semi = builder.build_loss(loss_semi)
    ...
    @auto_fp16(apply_to=('teache_input', 'input'))
    def forward(self,
                teacher_input=None,
                input=None,
                target=None,
        ...
        """
        if return_loss:
            return self.forward_train(teacher_input, input, target, target_weight, metas,
                                      **kwargs)
        else:
            return self.forward_test(input, metas, **kwargs)

    def forward_train(self, teacher_input, input, target, target_weight, metas, **kwargs):
        ...
        # get teacher output
        teacher_output = self.teacher_model.backbone(teacher_input)
