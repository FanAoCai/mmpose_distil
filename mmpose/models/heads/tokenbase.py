...
class TokenPose_TB_base(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, 
        ...
        #self.patch_size1 = patch_size[0]
        #self.patch_size2 = patch_size[1]
    ...
    def forward(self, feature, mask = None):
        ...
        x = rearrange(feature, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1 = p[0], p2 = p[1])    #转置
...
class DistilPose_base(nn.Module):
    ...
    def _make_sine_position_embedding(self, d_model, temperature=10000,
                                      scale=2 * math.pi):
        h, w = self.pe_h, self.pe_w
        area = torch.ones(1, h, w)  # [b, h, w]       #（1， 16， 16）
        y_embed = area.cumsum(1, dtype=torch.float32)    #按照横向累计求和
        x_embed = area.cumsum(2, dtype=torch.float32)    #按照纵向累计求和

        one_direction_feats = d_model // 2    #96

        eps = 1e-6
        y_embed = y_embed / (y_embed[:, -1:, :] + eps) * scale    #抽取最后一行保持维度不变
        x_embed = x_embed / (x_embed[:, :, -1:] + eps) * scale    #抽取最后一列保持维度不变

        dim_t = torch.arange(one_direction_feats, dtype=torch.float32)
        dim_t = temperature ** (2 * (dim_t // 2) / one_direction_feats)

        pos_x = x_embed[:, :, :, None] / dim_t      #按列单独分开增大一个维度
        pos_y = y_embed[:, :, :, None] / dim_t
        pos_x = torch.stack(
            (pos_x[:, :, :, 0::2].sin(), pos_x[:, :, :, 1::2].cos()), dim=4).flatten(3)
        pos_y = torch.stack(
